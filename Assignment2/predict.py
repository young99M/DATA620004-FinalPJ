...
if __name__ == "__main__":
    model_type = 'no-pretrain'
    # model_type = 'imagenet'
    # model_type = 'maskRCNNpretrain'
    frcnn = FRCNN(model_path='/home/newdisk/zzy/logs/' + model_type + '/best_epoch_weights.pth')
    #----------------------------------------------------------------------------------------------------------#
    #   mode用于指定测试的模式：
    ...
    if mode == "predict":
        ...
        while True:
            ...
            try:
                ...
                r_image = frcnn.detect_image(dir_save_path, model_type, image_id, image, crop = crop, count = count)
                # r_image.show()
                r_image.save(os.path.join(dir_save_path, model_type + '_test_' + image_id))
